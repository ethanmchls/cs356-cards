    ...
    <div className="rating rating-md rating-half">
      {Array.from({ length: 10 }).map((_, index) => (
        <input
          ...
          className={`bg-secondary mask mask-star-2 ${index % 2 === 0 ? 'mask-half-1' : 'mask-half-2'}`}
      ...
      <div className="modal-box bg-base-100 rounded-box !max-w-2xl">
        <div className="flex flex-row text-sm">
          ...
          <figure className="aspect-square w-auto ml-auto">
            <img src={`${game.image}`} alt="" className="hidden sm:block xs:w-0 sm:w-64" />
          </figure>
        </div>
        <p className="py-4">
        ...
        <div className="flex items-center">
          <a href={game.purchaseLink} target="_blank" rel="noreferrer" className="underline text-accent flex items-center">
            Purchase game
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-box-arrow-up-right ml-1" viewBox="0 0 16 16">
